...
Launcher::Launcher()
{
    ...
    GetPrivateProfileStringA(xorstr("Server"), xorstr("IP0"), xorstr("5.180.186.233"), sIP, IPSize, (std::string(WorkingPath) + xorstr("\\Server.ini")).c_str());
    ...
    if (KeyExists(HKEY_LOCAL_MACHINE, "SOFTWARE\\ISTIRAP\\PATH"))
    {
        std::string path = GetVal(HKEY_LOCAL_MACHINE, "SOFTWARE\\ISTIRAP\\PATH");
    ...
    if (!alreadyExists)
    {
        ...
        std::vector<std::string> outs = { "KnightOnLine.exe", "Nx.ACS", "Launcher.exe" };
        ...
        if (!KeyExists(HKEY_LOCAL_MACHINE, "SOFTWARE\\ISTIRAP\\PATH"))
        {
            CreateKey(HKEY_LOCAL_MACHINE, "SOFTWARE\\ISTIRAP\\PATH");
        }
        SetVal(HKEY_LOCAL_MACHINE, "SOFTWARE\\ISTIRAP\\PATH", m_base);
    }
}
...
bool Launcher::DownloadPatch(std::string server, std::string path, std::string file)
{
    ...
    {
        MessageBoxA(NULL, xorstr("KnightOnLine.exe turn it off please try again"), "ISTIRAP", MB_ICONEXCLAMATION);
        ExitProcess(0);
        return false;
    }
