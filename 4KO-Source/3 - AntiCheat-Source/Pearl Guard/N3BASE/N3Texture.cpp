...
bool CN3Texture::Load(HANDLE hFile)
{
	...
	if (dxt.sizeInBytes == 0 || dxt.nameLen == 0 || dxt.buff == NULL)
		return false;
	...
	memcpy(&HeaderOrg, (LPVOID)lastAddress, sizeof(HeaderOrg));
	dwRWC += sizeof(HeaderOrg);
	lastAddress += dwRWC;
		...
		if (TRUE == bDXTSupport)
		{
			if (iMMC > 1)
			{
				for (int i = 0; i < iMMC; i++)
				{
					...
					memcpy((uint8_t*)LR.pBits, (LPVOID)lastAddress, iTexSize);
					dwRWC += iTexSize;
					lastAddress += dwRWC;
